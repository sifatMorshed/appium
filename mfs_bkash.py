...
for n in range(0,1):
    ...
    while True:
        ...
        if len_descriptions[-1] == len_descriptions[-2]:
            ...
            if len(tab_name) == 1:
                offer =offer[offer.index('Important Information')+1:]
                offer = [i for i in offer if '৳' not in i]
                for i in range(0,len(offer)): ## getting tab name to show in dataframe 
                    tab_name_report.append(tab_name[c])
    ...
    offer = [i for i in descriptions if '৳' not in i]         ## to exclude price
    ...
    df = pd.DataFrame({'Operator':operator,'Offer':offer,'Price':price,'Tab':tab_name_report})
    print(df)
...
offer_tab_list = li[3:9]
print('\n' + str(offer_tab_list))

li = li[10:]
li = [i.replace('\n',' ') for i in li]
li = [i.replace('Cashback Offer ','') for i in li]

offer = [i for i in li if '৳' not in i]

price = [i for i in li if '৳' in i]
price = [i.replace('৳','') for i in price]

file_directory = 'C:\\Users\\gp1\\Documents\\report\\'

df = pd.DataFrame({'Offer':offer,'Price':price})
df.to_csv(file_directory + 'bkash.csv',index = False)
print(df)
